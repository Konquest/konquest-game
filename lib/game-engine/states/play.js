...
Play.prototype.preload = function () {
  ...
  this.game.onSync.add(this._onGameSync, this)
}
...
Play.prototype.shutdown = function () {
  ...
  this.game.onSync.remove(this._onGameSync, this)
}
...
Play.prototype._onPlayerUpdate = function (playerState) {
  ...
  if (!player) {
    return console.warn(new Error('Update: Player does not exist'))
  }

  player.controls = playerState.controls
}

Play.prototype._onGameSync = function (playerStates) {
  ...
  for (var i = 0; i < playerStates.length; i++) {
    ...
    if (!player) {
      return console.warn(new Error('Sync: Player does not exist'))
    }
